...
操作台支持 `NaviCat UI 模拟模式`、`普通 SQL 语句模式` 和 `Terminal 模式`。NaviCat 模式会显示连接窗口、查询标签、对象工具栏和数据网格；Terminal 模式偏命令行输入。首次进入 `/console.html` 会显示使用提示，并在当前浏览器记录已读状态。右上角设置可以切换模式、显示可视化数据，以及打开可视化转 SQL 侧栏。
